(function () {
	...
	var qt = function ($uibModalInstance) {
		...
		that.q_texts = '<article style="display: block;"><p><b>Merge/Edit/Delete Items</b> – Please check out our instruction video <a href="https://youtu.be/uGnxBV3Xkws" target="_blank">here</a>.</li></ul></p></article>';
    ...
    function inventoryMergeController(api, $state, auth, localStorageService, alertService, SweetAlert, $rootScope, restaurant, $uibModal, core, $scope, $q) {
		...
		that.getVendorList = function () {
            var deferred = $q.defer();
			that.api.get_chosen_vendors(that.restaurant_id.restaurant_id, {vendor_type_id: 2}).then(function (res) {
				try {
    ...
    angular.module('inspinia').component('alcoholInventoryMergeComponent', {
        templateUrl: 'js/components/alcoholSetup/inventoryMerge/inventoryMerge.html',
