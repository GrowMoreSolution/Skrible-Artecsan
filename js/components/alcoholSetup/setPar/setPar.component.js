(function () {
    ...
    function setParController(api, $state, auth, core, localStorageService, alertService, SweetAlert, restaurant, $rootScope, $scope, $q) {
			...
			for(var pd in that.par_data){
				...
				if(pdi.substitute_for == null){
					...
					if(pdi.minimum_order_type == 'Case'){
						pdi.suggested_par_units = Math.ceil(pdi.suggested_par_units / pdi.pack);
					}
					...
					if(pdi.actual_order_unit != pdi.minimum_order_type && pdi.actual_par_units != null){
						if(pdi.minimum_order_type == 'Case'){
							...
							if(that.category != 'Wine'){
								pdi.actual_par_units = pdi.actual_par_units * pdi.pack;
							}
						}
					}
					
					if(that.category == 'Wine'){		//only for display purpose
						if(pdi.minimum_order_type != 'Case'){
							pdi.suggested_par_units = Math.ceil(pdi.suggested_par_units / pdi.pack);
							pdi.minimum_order_type = 'Case';
						}
					}
		...
		that.getOH = function () {		
			that.api.alcohol({'RestaurantId': that.restaurant_id.restaurant_id, 'Category': that.category, 'caller': 'oh_report'}).then(function (res) {
				if(res){
					...
					for(var pt in that.processed_par_items){
						...
						for(var ot in that.oh_table_data){
							if(that.oh_table_data[ot].vendor_sku_id == that.processed_par_items[pt].vendor_sku_id){
								that.processed_par_items[pt].units_on_hand = that.oh_table_data[ot].units_on_hand;
								found_item = true;
								break;
							}
							if(!found_item){
		...
		that.getSuggestedOrder = function () {
			that.api.get_suggested_orders({"RestaurantId": that.restaurant_id.restaurant_id, 'inventory_type_id': 2}).then(function (res) {
				if(res){
			...
			if(that.dataChanged){
                ...
                function (sres) {
					if (sres) {
						...
						that.api.get_par({'category': that.category, 'inventory_type_id': 2, 'get_notif_status': true}).then(function (res) {
							if (res) {
		...
		that.saveActualPar = function () {
			that.api.update_par({'inventory_type_id': 2, 'pars_data': that.par_data.map(function (x) {return {vendor_sku_id: x.vendor_sku_id, actual_par_units: x.actual_par_units, order_unit: x.bu_minimum_order_type, alert_if_low: x.alert_if_low, notify_units: x.notify_units}})}).then(function (res) {
        ...
        that.$onInit = function () {
			that.api.get_active_SKU_categories({'inventory_type_id': 2}).then(function (res) {
    ...
    angular.module('inspinia').component('setParComponent', {
        templateUrl: 'js/components/alcoholSetup/setPar/setPar.html',
