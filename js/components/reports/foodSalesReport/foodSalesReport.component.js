(function () {
    ...
    function foodSalesReportController(api, $scope,$state, auth, core, localStorageService, restaurant, $rootScope) {
		...
		that.loadLateralChart = function () {
            ...
            that.api.lateral_chart_api({'From': fd, 'To': td, 'RestaurantId': that.restaurant_id.restaurant_id, 'Category': 'Food', 'inventory_type_id': 1}).then(function (res) {
				that.loading = false;
                var color = [];
...
//                        rescale: true
		...
		that.loadPMIXWeeklyData = function () {
			...
			that.loading = true;
			that.api.pmix({'RestaurantId': that.restaurant_id.restaurant_id, 'inventory_type_id': 1, 'category': that.category == '-- All --' ? 'All' : that.category == 'Food, Lunch & Dinner' ? 'Food' : that.category, 'mode': 'weekly'}).then(function (res) {
				that.loading = false;
		...
		that.loadPMIXTrendData = function () {
			...
			that.loading = true;			
			that.api.pmix({'RestaurantId': that.restaurant_id.restaurant_id, 'inventory_type_id': 1, 'category': that.category == '-- All --' ? 'All' : that.category == 'Food, Lunch & Dinner' ? 'Food' : that.category, 'mode': 'trend'}).then(function (res) {
				that.loading = false;
				that.pmix_data = res.data.data.pMIX;
		...
		that.loadDailySalesData = function () {
			...
			that.api.sales_current_to_last_period_report({'RestaurantId': that.restaurant_id.restaurant_id, 'inventory_type_id': 1, 'mode': that.daily_sales_mode.mode}).then(function (res) {
				that.loading = false;
		...
		that.SalesSummary = function () {
            ...
            that.api.salesSummary({'From': fd, 'To': td, 'RestaurantId': that.restaurant_id.restaurant_id, 'Category': 'Food'}).then(function (res) {
                if (res) {
                    that.sales = {'Food': 0, 'Total': 0};
                    var sales_total = 0;
                    for (var i = 0; i < res.data.data.salesSummary.length; i++)
                    {
                        if (res.data.data.salesSummary[i].category == 'Food') {
                            that.sales['Food'] = res.data.data.salesSummary[i].sales;
        ...
        that.$onInit = function () {
			//that.api.get_active_SKU_categories({'inventory_type_id': 1}).then(function (res) {
				...
				that.categories.push({'category': 'Food, Lunch & Dinner'});
    ...
    foodSalesReportController.$inject = ['api','$scope', '$state', 'auth', 'core', 'localStorageService', 'restaurant', '$rootScope'];

    angular.module('inspinia').component('foodSalesReportComponent', {
        templateUrl: 'js/components/reports/foodSalesReport/foodSalesReport.html',
        controller: foodSalesReportController,
        controllerAs: '$ctr',
        bindings: {}
    });
