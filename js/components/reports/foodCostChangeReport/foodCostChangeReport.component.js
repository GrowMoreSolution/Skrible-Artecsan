(function () {
    ...
    function foodCostChangeReportsController(api, $state, auth, core, localStorageService, restaurant, $rootScope, $scope) {
        ...
        that.costChangeGraph = function () {
            ...
            that.api.costChangeCostCompare({'start_date': fd, 'end_date': td, 'RestaurantId': that.restaurant_id.restaurant_id, 'inventory_type_id': 1, 'items': that.selectedItems.map(function (x){return x.id})}).then(function (res) {
		...
		that.costChangeDetails = function () {
			var params = {'inventory_type_id': 1, 'category': that.details_category.id == -1 ? '%' : that.details_category.category, 'sub_category': that.subcategory.id == -1 ? '%' : that.subcategory.sub_category, 'item': that.item.id == -1 ? '%' : that.item.id, 'mode': that.details_mode.type};
        ...
        that.$onInit = function () {
			that.mode = 'details';
			that.export_loading = false;
			Promise.all([api.get_vendors_categories({is_restaurant_used_only: 1, inventory_type_id: 1}), core.getRefbooks(), that.getChosenVendors(), that.getInventoriesByVendor()]).then(function(response) {
    ...
    foodCostChangeReportsController.$inject = ['api', '$state', 'auth', 'core', 'localStorageService', 'restaurant', '$rootScope', '$scope'];

    angular.module('inspinia').component('foodCostChangeReportComponent', {
        templateUrl: 'js/components/reports/foodCostChangeReport/foodCostChangeReport.html',
        controller: foodCostChangeReportsController,
        controllerAs: '$ctr',
        bindings: {}
    });
