(function () {
	...
	var modalController = function ($uibModalInstance, api, alertService) {
        ...
        that.sendPDF = function() {
			...
			that.api.on_hand_email({'start_date': fd, 'end_date': td}).then(function (res) {	
				//let it run
            });
    ...
    function usageVsPurchaseController(api, $state, $filter, auth, core, localStorageService, restaurant, $uibModal, $rootScope, $scope) {
        ...
        that.PurchasesVsUsage = function () {
			...
			that.api.PurchaseVsUsage({'From': fd, 'To': td, 'RestaurantId': that.restaurant_id.restaurant_id, 'inventory_type_id': 2}).then(function (res) {	
				if(res){
        ...
        that.VendorTable = function () {
            ...
            that.api.VendorDetail({'From': fd, 'To': td, 'RestaurantId': that.restaurant_id.restaurant_id, 'inventory_type_id': 2}).then(function (res) {
				swal.close()
				if(res){
        ...
        that.$onInit = function () {
			Promise.all([api.get_vendors_categories({is_restaurant_used_only: 1, inventory_type_id: 2}), core.getRefbooks()]).then(function(response) {
    ...
    usageVsPurchaseController.$inject = ['api', '$state', '$filter', 'auth', 'core', 'localStorageService', 'restaurant', '$uibModal', '$rootScope', '$scope'];

    angular.module('inspinia').component('usageVsPurchaseComponent', {
        templateUrl: 'js/components/reports/usageVsPurchase/usageVsPurchase.html',
        controller: usageVsPurchaseController,
        controllerAs: '$ctr',
        bindings: {}
    });
